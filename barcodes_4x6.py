...
def main():
    ...
    pagenum = 1
    while lefttoprint > 0:
        thischunk = barcodes[12*(pagenum-1):12*pagenum]
        thisfilename = "out/4by6_"+str(pagenum).zfill(5)+".png"
        pageim = placeLabels_onPage(thischunk)
        cv2.imwrite(thisfilename, pageim)
        call("convert -units PixelsPerInch -density 203 \""+thisfilename+"\" \""+thisfilename+"\"", shell = True)
        lefttoprint -= 12
        pagenum += 1
...
def placeLabels_onPage(labellist):
    ...
    step_y = 300
    step_x = 270
    pageim = np.ones((1218, 812, 3), np.uint8)*255
    ...
    #import pdb
    #pdb.set_trace()
    while pagepos < 12 and pagepos < len(labellist):
        x_pos = pagepos // 4
        y_pos = pagepos % 4
        thislabel_im = labellist[4*x_pos+y_pos]
...
def makeBarcode(myurl, mycaption = ""):
    ...
    baseimg[20:qr_size+20, 20:-20] = qrimg
    #
    ...
    drawthis.text((50, 250), mycaption, font = ImageFont.truetype('/usr/share/fonts/truetype/msttcorefonts/arial.ttf', 20), fill = (0, 0, 0))
